    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        {Object.entries(logCounts).map(([type, count]) => (
          <div
            key={type}
            className={`metric-card cursor-pointer ${filter === type ? 'border-ats-primary' : ''}`}
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="text-lg font-semibold text-white">
            Server Logs ({filteredLogs.length})
          </h3>
          <FileText className="w-5 h-5 text-ats-primary" />
        </div>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">
                {search || filter !== 'all' 
                  ...
                  : 'Server logs will appear here as activity occurs'
                }
              </p>
            </div>
          )}
